(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e){if(t.type!==`childList`)continue;for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();const e=`modulepreload`,t=function(e){return`/front_6th_chapter1-1/`+e},n={},r=function(r,i,a){let o=Promise.resolve();if(i&&i.length>0){let r=document.getElementsByTagName(`link`),s=document.querySelector(`meta[property=csp-nonce]`),c=s?.nonce||s?.getAttribute(`nonce`);function l(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}o=l(i.map(i=>{if(i=t(i,a),i in n)return;n[i]=!0;let o=i.endsWith(`.css`),s=o?`[rel="stylesheet"]`:``,l=!!a;if(l)for(let e=r.length-1;e>=0;e--){let t=r[e];if(t.href===i&&(!o||t.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${i}"]${s}`))return;let u=document.createElement(`link`);if(u.rel=o?`stylesheet`:e,o||(u.as=`script`),u.crossOrigin=``,u.href=i,c&&u.setAttribute(`nonce`,c),document.head.appendChild(u),o)return new Promise((e,t)=>{u.addEventListener(`load`,e),u.addEventListener(`error`,()=>t(Error(`Unable to preload CSS for ${i}`)))})}))}function s(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return o.then(e=>{for(let t of e||[]){if(t.status!==`rejected`)continue;s(t.reason)}return r().catch(s)})};async function i(e={}){let{limit:t=20,search:n=``,category1:r=``,category2:i=``,sort:a=`price_asc`}=e,o=e.current??e.page??1,s=new URLSearchParams({page:o.toString(),limit:t.toString(),...n&&{search:n},...r&&{category1:r},...i&&{category2:i},sort:a}),c=await fetch(`/api/products?${s}`);return await c.json()}async function a(e){let t=await fetch(`/api/products/${e}`);return await t.json()}async function o(e,t,n=4){let r=new URLSearchParams({category2:e,limit:n.toString(),exclude:t}),i=await fetch(`/api/products?${r}`);return await i.json()}async function s(){let e=await fetch(`/api/categories`);return await e.json()}const c=e=>{let t=[10,20,50,100].map(t=>`<option value="${t}" ${t===e?`selected`:``}>${t}개</option>`);return`
  ...
  `,ge(e),T(),b()},ge=e=>{let t=document.getElementById(`back-button`);t&&t.addEventListener(`click`,async()=>{await $()});let n=document.getElementById(`quantity-increase`),r=document.getElementById(`quantity-decrease`),i=document.getElementById(`quantity-input`);n&&n.addEventListener(`click`,()=>{let e=parseInt(i.value),t=parseInt(i.max);e<t&&(i.value=e+1)}),r&&r.addEventListener(`click`,()=>{let e=parseInt(i.value);e>1&&(i.value=e-1)}),i&&i.addEventListener(`change`,()=>{let e=parseInt(i.value),t=parseInt(i.max);e<1?i.value=1:e>t&&(i.value=t)});let a=document.getElementById(`add-to-cart-btn`);a&&a.addEventListener(`click`,()=>{let t=parseInt(i.value),n={...e,quantity:t};_(n)});let o=document.getElementById(`go-to-product-list`);o&&o.addEventListener(`click`,async()=>{await Q()});let s=document.querySelectorAll(`.breadcrumb-link`);s.forEach(e=>{e.addEventListener(`click`,async t=>{t.preventDefault();let n=e.getAttribute(`data-category1`),r=e.getAttribute(`data-category2`);n?await X(`/?category1=${n}`):r&&await X(`/?category2=${r}`)})});let c=document.querySelectorAll(`.related-product-card`);c.forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.getAttribute(`data-product-id`);t&&await Z(t)})})},_e=async e=>{try{z(K);let t=await a(e);if(!t)throw Error(`상품을 찾을 수 없습니다.`);let n=[];if(t.category2)try{let r=await o(t.category2,e,4);n=r.products||r||[]}catch(e){console.error(`관련 상품 로딩 실패:`,e)}he(t,n)}catch(e){console.error(`상품 상세 로딩 실패:`,e),K.innerHTML=`
    ...
    `;let t=document.getElementById(`error-back-button`);t&&t.addEventListener(`click`,async()=>{await $()})}},ve=()=>`
        <main class="max-w-md mx-auto px-4 py-4">
    ...
    `,q=`/front_6th_chapter1-1`,J=e=>e.startsWith(q)?e.slice(21):e,Y=async e=>{let t=J(e)||`/`;if(t.startsWith(`/product/`)){let e=t.split(`/product/`)[1];if(e){await _e(e);return}}if(t===`/`||t===``){await fe();return}document.body.innerHTML=ve()},ye=async()=>{await Y(window.location.pathname),window.addEventListener(`popstate`,async()=>{await Y(window.location.pathname)}),document.addEventListener(`click`,async e=>{let t=e.target.closest(`a[data-link]`);if(t){e.preventDefault();let n=t.getAttribute(`href`),r=q+n;window.history.pushState({},``,r),await Y(r)}})},X=async e=>{let t=q+e;window.location.pathname!==t&&(window.history.pushState({},``,t),await Y(t))},Z=async e=>{let t=`/product/${e}`;await X(t)},Q=async()=>{await X(`/`)},$=async()=>{window.history.length>1?window.history.back():await Q()},be=`/front_6th_chapter1-1/`,xe=()=>r(async()=>{let{worker:e}=await import(`./browser-D14UcMCg.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`,serviceWorker:{url:`${be}mockServiceWorker.js`}}));var Se=class{constructor(){this.isInitialized=!1}async initialize(){if(!this.isInitialized)try{await ye(),this.isInitialized=!0}catch(e){console.error(`애플리케이션 초기화 실패:`,e)}}async restart(){this.isInitialized=!1,await this.initialize()}};const Ce=new Se;async function we(){await Ce.initialize()}xe().then(we);export{E as b,D as c,O as d,g as e,x as f,v as g,S as h,_ as i,T as j,y as k,C as l,b as m};